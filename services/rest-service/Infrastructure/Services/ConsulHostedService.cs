...
public class ConsulHostedService : IHostedService
{
    ...
    {
        var serviceName = _configuration["Consul:ServiceName"] ?? "rest-service";
        var serviceHost = Environment.GetEnvironmentVariable("HOSTNAME") ?? _configuration["Consul:ServiceHost"] ?? "rest-service";
        var servicePort = int.Parse(_configuration["Consul:ServicePort"] ?? "80");

        _registrationId = $"{serviceName}-{serviceHost}-{Guid.NewGuid():N}";
